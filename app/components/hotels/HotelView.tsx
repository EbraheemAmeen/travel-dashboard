...
import { getRoomTypes, deleteRoomType } from '@/app/actions/hotels/room-types/getRoomTypes';
import { deleteRoomType as deleteRoomTypeAction } from '@/app/actions/hotels/room-types/deleteRoomType';
...
export default function HotelView({
  ...
  const { data: hotel, isLoading: loadingHotel, error: hotelError } = useQuery({
    queryKey: ['hotel', hotelId],
    queryFn: () => getHotelById(hotelId),
  });

  const { data: roomTypesData, isLoading: loadingRoomTypes, error: roomTypesError } = useQuery({
  ...
  const deleteMutation = useMutation({
    mutationFn: ({ hotelId, roomTypeId }: { hotelId: number; roomTypeId: number }) => 
      deleteRoomTypeAction(hotelId, roomTypeId),
    onSuccess: () => {
    ...
    return (
      <div className="p-12 min-h-screen bg-gray-900 text-white">
        <div className="text-center">
          ...
          <p className="mt-4 text-gray-400">Loading hotel information...</p>
        </div>
      </div>
    );
  }
  ...
  if (hotelError || !hotel) {
    return (
      <div className="p-12 min-h-screen bg-gray-900 text-white">
        <div className="text-center">
          <h1 className="text-2xl font-bold text-red-500 mb-4">Error Loading Hotel</h1>
          <p className="text-gray-400">Failed to load hotel information.</p>
        </div>
      </div>
    );
  }
        ...
        <div className="mb-8">
          <div className="flex items-center gap-6 mb-6">
            ...
            <div>
              ...
              <div className="flex items-center gap-4 text-gray-400">
                <div className="flex items-center gap-1">
                  {[...Array(5)].map((_, i) => (
                    <svg
                      key={i}
                      className={`w-5 h-5 ${i < hotel.stars ? 'text-yellow-400' : 'text-gray-600'}`}
                      fill="currentColor"
                      viewBox="0 0 20 20"
                    >
              ...
              {hotel.description && (
                <p className="text-gray-300 mt-2 max-w-2xl">{hotel.description}</p>
              )}
            </div>
          </div>
        </div>

...
export default function AttractionsList({
  ...
  const [deleteModal, setDeleteModal] = useState<{ open: boolean; attraction: any | null }>({ open: false, attraction: null });
  const [deleteError, setDeleteError] = useState<string | null>(null);

  const { data: attractionsData, isLoading, error } = useQuery({
    queryKey: ['attractions', cityId, page, limit, orderBy, orderDir, search, poiTypeId, minPrice, maxPrice, isActive],
  ...
  const totalPages = attractionsData?.totalPages || 1;

  const handleDelete = (attraction: any) => {
    setDeleteModal({ open: true, attraction });
    setDeleteError(null);
  };

  const confirmDelete = () => {
    if (deleteModal.attraction) {
      deleteMutation.mutate({ attractionId: deleteModal.attraction.id });
    }
  };
    ...
    return (
      <div className="p-12 min-h-screen bg-gray-900 text-white">
        <div className="text-center">
          ...
          <p className="mt-4 text-gray-400">Loading attractions...</p>
        </div>
      </div>
    );
  }
  ...
  if (error) {
    return (
      <div className="p-12 min-h-screen bg-gray-900 text-white">
        <div className="text-center">
          <h1 className="text-2xl font-bold text-red-500 mb-4">Error Loading Attractions</h1>
          <p className="text-gray-400">Failed to load attractions.</p>
        </div>
      </div>
    );
  }
    ...
    <div className="p-12 min-h-screen bg-gray-900 text-white">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex justify-between items-center mb-8">
          <h1 className="text-3xl font-bold text-white">Attractions</h1>
          <Link
            href={`/cities/${cityId}/attractions/new`}
        ...
        <div className="bg-gray-800 rounded-lg p-6 mb-8">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            <div>
              <label className="block text-sm font-medium text-gray-300 mb-2">POI Type</label>
              <select
                value={poiTypeId || ''}
                onChange={(e) => {
                  const url = new URL(window.location.href);
                  if (e.target.value) {
                    url.searchParams.set('poiTypeId', e.target.value);
                  } else {
                    url.searchParams.delete('poiTypeId');
                  }
                  window.location.href = url.toString();
                }}
                className="w-full bg-gray-700 border border-gray-600 rounded-lg px-3 py-2 text-white"
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {attractions.map((attraction: any) => (
            <div key={attraction.id} className="bg-gray-800 rounded-lg overflow-hidden border border-gray-700">
              ...
              <div className="p-6">
                ...
                <p className="text-gray-400 text-sm mb-4">{attraction.description}</p>
                <div className="space-y-2 mb-4">
                  <div className="flex justify-between text-sm">
                    <span className="text-gray-400">Type:</span>
                    <span className="text-white">{attraction.poiType?.name || 'N/A'}</span>
                  </div>
                  <div className="flex justify-between text-sm">
                  ...
                  <div className="flex justify-between text-sm">
                    ...
                    <span className="text-white">{attraction.avgDuration}</span>
                  </div>
                </div>
                <div className="flex gap-2">
                  <Link
                    href={`/cities/${cityId}/attractions/${attraction.id}/edit`}
        ...
        {deleteModal.open && (
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div className="bg-gray-800 rounded-lg p-6 max-w-md w-full mx-4">
              <h3 className="text-xl font-bold text-white mb-4">Delete Attraction</h3>
              <p className="text-gray-300 mb-6">
                Are you sure you want to delete "{deleteModal.attraction?.name}"? This action cannot be undone.
              </p>
              {deleteError && (
              ...
              <div className="flex gap-4">
                <button
                  onClick={() => setDeleteModal({ open: false, attraction: null })}
